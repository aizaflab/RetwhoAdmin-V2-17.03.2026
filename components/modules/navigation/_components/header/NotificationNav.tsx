  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="relative p-2 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 rounded-full transition-all duration-300 cursor-pointer outline-none group"
        onClick={() => setOpenDropdown(!openDropdown)}
      >
        <Bell className="h-5 w-5 group-hover:animate-[wiggle_1s_ease-in-out_infinite]" />
        {unreadCount > 0 && (
          <span className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-red-500 ring-2 ring-white dark:ring-slate-900 shadow-[0_0_8px_rgba(239,68,68,0.8)] animate-pulse"></span>
        )}
      </button>

      {/* Dropdown Panel */}
      {openDropdown && (
        <div className="absolute right-0 top-12 w-[380px] sm:w-[420px] bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.1)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-white/40 dark:border-white/10 z-50 overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-top-4 duration-300 origin-top-right">
          {/* Header */}
          <div className="relative px-5 py-4 border-b border-slate-200/50 dark:border-slate-800/50 overflow-hidden">
            {/* Header Background Glow */}
            <div className="absolute top-0 left-0 w-full h-[150px] bg-linear-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 blur-3xl rounded-t-2xl pointer-events-none -z-10"></div>

            <div className="flex items-center justify-between mb-1 relative z-10">
              <h3 className="text-lg font-bold bg-clip-text text-transparent bg-linear-to-r from-slate-900 to-slate-600 dark:from-white dark:to-slate-300">
                Notifications
              </h3>
              {unreadCount > 0 && (
                <button
          ...
          <div className="px-5 py-3 border-b border-slate-200/50 dark:border-slate-800/50 relative">
            ...
            {canScrollLeft && (
              <button
                onClick={() => scroll("left")}
                className="absolute left-2 top-1/2 -translate-y-1/2 z-20 h-7 w-7 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center shadow-md transition-all hover:scale-110 text-slate-700 dark:text-slate-300 cursor-pointer"
            ...
            {canScrollRight && (
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 z-20 h-7 w-7 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center shadow-md transition-all hover:scale-110 text-slate-700 dark:text-slate-300 cursor-pointer"
                ...
                return (
                  <button
                    ...
                    className={`flex items-center cursor-pointer gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-300 whitespace-nowrap ${
                      isActive
                        ? "bg-linear-to-r from-blue-600 to-indigo-600 text-white shadow-[0_0_15px_rgba(79,70,229,0.4)] scale-105"
              ...
              <div className="p-2 space-y-1">
                {filteredNotifications.map((notification) => {
                  ...
                  return (
                    <div
                      key={notification.id}
                      className={`relative group p-3 rounded-xl transition-all duration-300 cursor-pointer overflow-hidden ${
                        !notification.read
                    ...
                    >
                      ...
                      {!notification.read && (
                        <div className="absolute inset-0 rounded-xl border border-blue-200 dark:border-blue-500/30 opacity-100 transition-opacity"></div>
                      )}
